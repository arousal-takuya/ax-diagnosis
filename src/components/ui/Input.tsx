...
export default function Input({ label, error, className = '', id, ...props }: InputProps) {
    ...
    return (
        <div className="w-full">
            ...
            <input
                id={inputId}
                className={`w-full rounded border border-[var(--border)] bg-white px-3 py-2 text-sm text-[var(--foreground)] placeholder-gray-400 transition-colors focus:border-[var(--primary)] focus:outline-none focus:ring-1 focus:ring-[var(--primary)] disabled:bg-gray-100 disabled:cursor-not-allowed ${error ? 'border-[var(--error)] focus:border-[var(--error)] focus:ring-[var(--error)]' : ''
                    } ${className}`}
                {...props}
            />
